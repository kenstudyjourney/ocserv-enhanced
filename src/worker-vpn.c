...
static int hello_hook_log_sni(gnutls_session_t session,
                              unsigned int htype,
                              unsigned when,
                              ...
                              const gnutls_datum_t *msg)
{
    ...
    // Only run after client hello is parsed
    if (htype != GNUTLS_HANDSHAKE_CLIENT_HELLO || when != GNUTLS_HOOK_POST)
        return 0;

    char sni[256];
    size_t sni_size = sizeof(sni);

    int ret = gnutls_server_name_get(session, sni, &sni_size, NULL, 0);

    if (ret < 0 || sni_size == 0) {
        oclog(ws, LOG_INFO, "Client did not provide SNI");
        return 0; // Do not abort handshake
    }

    if (sni_size >= sizeof(sni))
        sni_size = sizeof(sni) - 1;

    sni[sni_size] = '\0';

    oclog(ws, LOG_INFO, "Client SNI: '%s' (size %zu)", sni, sni_size);

    return 0; // Always return 0 to continue handshake
}
...
void vpn_server(struct worker_st *ws)
{
    ...
    if (ws->conn_type != SOCK_TYPE_UNIX) {
		...
		if (!HAVE_VHOSTS(ws)) {
			...
			// oclog(ws, LOG_INFO, "!HAVE_VHOSTS, credentials set");
		} else {
			// multiple vhosts
			gnutls_handshake_set_hook_function(
				session, GNUTLS_HANDSHAKE_CLIENT_HELLO,
				GNUTLS_HOOK_POST, hello_hook_log_sni);
			// oclog(ws, LOG_INFO, "HAVE_VHOSTS, POST hook registered");
		}
		// fprintf(stderr, "Debug: !HAVE_VHOSTS(ws) line passed\n");
